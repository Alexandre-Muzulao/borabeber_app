...
function parseCervejaAdm(ADMBEERS){
  if (ADMBEERS !== undefined){
    for (i = 0; i < ADMBEERS.length; i++){
      ...
      if (isPar(i) == 'par'){
        ...
        ADMBEERS[i].imgBeerPar = URLIMAGEBEERS + ADMBEERS[i].imgBeer
        ...
        // Limpa os os impares do mesmo indice
        ...
        ADMBEERS[i - 1].imgBeerImpar = URLIMAGEBEERS + ADMBEERS[i].imgBeer
